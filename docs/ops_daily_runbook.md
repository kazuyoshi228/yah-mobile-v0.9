...
|---|---|---|---|
| **朝（必須）** | eSIMAccess **残高**確認 | /admin（残高表示）or 通知履歴 | **オートチャージ稼働中**（$100以下で自動補充）。残高が異常に低い/オートチャージ失敗通知がないか確認だけ（§3） |
...
## 3. 残高運用（eSIMAccess）🟢 オートチャージ稼働中
**現状**: eSIMAccess 側に **オートチャージ（残高 $100 以下で自動補充）を設定済**。残高切れ→発行失敗→販売停止のリスクは平時ほぼ解消。**日常の手動チャージは不要**。

| 項目 | 値 |
|---|---|
| **オートチャージ**（eSIMAccess設定） | **残高 ≤ $100 で自動補充**（一次防衛） |
| 自動警告しきい値（当社コード） | **$20 未満**で `providerHealthCheck` がオーナー通知（`LOW_BALANCE_USD`）＝**オートチャージ失敗の検知バックストップ** |
| 販売停止ガード | API down / 発行失敗時に自動ON、回復で自動OFF |

**運用は「監視」だけ（手動チャージは例外時のみ）**
1. **平時**: 何もしない。オートチャージが維持。
2. **$20未満の警告が来たら**=オートチャージが効いていない兆候 → eSIMAccess ポータルで**支払い方法/オートチャージ設定/カード有効期限**を確認。必要なら手動 Deposit で応急補充。
3. 万一 $0 で販売停止した場合、補充後に残高回復→**ガード自動解除**（「eSIMAccess API が回復しました」通知）。解除されなければ [障害手順](./refund_incident_procedures.md)。

**チェック**: 月次でオートチャージの発動履歴・支払い方法の有効性を確認（カード期限切れ等でオートチャージが止まる事故を防ぐ）。連休・繁忙期前は上限/補充額が十分か見直す。
