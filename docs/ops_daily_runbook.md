...
|---|---|---|---|
| **朝（必須）** | eSIMAccess **残高**確認 | /admin（残高表示）or 通知履歴 | **< $50 で当日チャージ**（§3）。$20未満は警告済のはず |
...
| 項目 | 値 |
|---|---|
| 自動警告しきい値 | **$20 未満**（`LOW_BALANCE_USD`・1時間に1回オーナー通知） |
| 運用の安全在庫 | **$50 を下回ったらチャージ**（警告を待たず前倒し） |
...
**チェック**: 週次で「残高の減り方 vs 発行数」を見て、安全在庫（$50）が妥当か見直す。連休・繁忙期前は多めにチャージ。
