...
作成: 2026-07-07 ／ 更新: **2026-07-07（S7完了・方式Bログイン本番・柱2 Phase0完了/Phase1設計済み）** ／ 基準コミット: `8559f37`（dev=origin/v0.51）／ ステータス: **進行中（各フェーズ着手前に個別承認）**
...
- 🎯 **eSIMAccess をメインプロバイダ**、**Bappy/OMAX をフォールバック/既存eSIM継続**に（切替はconfig駆動＋カナリア）。

### フェーズと残To-Do
**Phase 0 — 契約・調査（✅完了）**
- [x] パートナー登録・API仕様確定（[esimaccess_api_notes.md](./esimaccess_api_notes.md)：全エンドポイント／HMAC署名／IP許可／cancel返金挙動）。
- [x] **日本パッケージ実機テスト＝問題なし**（**IIJ＝ドコモ回線・non-HK IP**）。→ eSIMAccessメイン化を確定。
- [x] 署名（RT-AccessCode or HMAC-SHA256）・送信元IP（既知5個）・cancel返金（未使用のみ残高返金）確認済み。

**Phase 1 — Provider 抽象（Bappy挙動不変・要承認）** … 📐 **設計済み**（[design_provider_abstraction.md](./design_provider_abstraction.md)）・実装承認待ち
- [ ] `functions/src/providers/types.ts`（`EsimProvider` IF: createEsim/getEsimDetail/topup/cancel?/queryBalance?・`getProvider`）
- [ ] `providers/bappy.ts`（既存 `bappy/*` の薄いラッパ）
- [ ] `fulfillEsim`／`esimRetryService`／`onEsimSyncRequested`／topup を `getProvider(...)` 経由へ
- [ ] `shared/types.ts` に `provider`/`providerRef`（既定 bappy・後方互換）
- [ ] 既存テスト41件＋新規 全通過＝**挙動不変の担保**
